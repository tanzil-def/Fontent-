...
export default function ManageBooks() {
  ...
  // ------------ FORM MODEL (image/mp4/pdf uploads) ------------
  const emptyForm = {
    title: "",
    copies: "",
    author: "",
  ...
  // file inputs (image/video/pdf)
  const handleFile = (e, kind) => {
    const file = e.target.files && e.target.files[0];
    if (!file) return;
    const url = URL.createObjectURL(file);
    if (kind === "image") {
      setForm((f) => ({ ...f, coverFile: file, coverUrl: url }));
    } else if (kind === "video") {
      setForm((f) => ({ ...f, clipFile: file, clipUrl: url }));
    } else if (kind === "pdf") {
      setForm((f) => ({ ...f, pdfFile: file, pdfUrl: url }));
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (mode === "edit" && editingIndex >= 0) {
      setDisplayed((prev) => {
        ...
        row.clip = form.clipUrl || row.clip || "";
            ...
            >
              ...
              <div className="px-6 py-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* Promo Clip (replaces Publisher) */}
                  <div>
                    <label className="block text-sm text-gray-700 mb-1">
                      Promo Clip (MP4, optional)
                    </label>
                    <input
                      type="file"
                      accept="video/mp4"
                  ...
                  {/* Rack & Status removed as requested */}
                </div>
              </div>
